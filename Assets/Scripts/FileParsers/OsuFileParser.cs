            ...
            try
            {
                switch (state)
                {
                    ...
                    case ParseState.TimingPoints:
                        ...
                        if (timingpoint[6] == "1") // if timing point is not inherited
                        {
                            timingPoint.SpeedMultiplier = (timingPoints.Count > 1) ? timingPoints[^1].SpeedMultiplier : 1f;
                            timingPoint.BPM = 60000f / float.Parse(timingpoint[1], System.Globalization.CultureInfo.InvariantCulture);
                        }
                        else  // if it is
                        {
                            timingPoint.SpeedMultiplier = -100f / float.Parse(timingpoint[1], System.Globalization.CultureInfo.InvariantCulture);
                            timingPoint.BPM = float.MinValue;
                        }
                    ...
                    case ParseState.HitObjects:
                        ...
                        if (fastPass)
                        {
                            ...
                            // If last note is hold, use release time
